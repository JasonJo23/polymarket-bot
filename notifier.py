...
def _money_emoji(value: float) -> str:
    if value > 0:
        return "🟢"
    if value < 0:
        return "🔴"
...
class TelegramNotifier:
    ...
    def notify_sell(self, question: str, pnl_pct: float, reason: str):
        ...
        title = "MYYNTI VOITOLLA" if pnl_pct > 0 else "MYYNTI TAPPIOLLA" if pnl_pct < 0 else "MYYNTI"
        msg = (
            f"<b>{emoji} {title}</b>\n"
            f"📊 {_text(question, 60)}\n"
            f"📈 P&L: <b>{pnl_pct:+.1%}</b>\n"
    ...
    def notify_bad_fill(
        ...
        msg = (
            ...
            f"📊 {_text(signal.get('question'), 60)}\n"
            f"🎯 Outcome: <b>{_text(signal.get('outcome'), 40)}</b>\n"
            f"🎚️ Pyydetty: {requested_price:.3f} | Toteutui: <b>{actual_price:.3f}</b>\n"
    ...
    def notify_cycle_summary(self, summary: dict):
        ...
        pnl_emoji = _money_emoji(daily_pnl)
        msg = (
            ...
            f"{pnl_emoji} Live PnL: {daily_pnl:+.2f} USDC\n"
            ...
            f"⏱️ Avg sykli: {float(summary.get('avg_cycle_seconds', 0.0) or 0.0):.1f}s | "
            f"📅 {_utc_time()}"
        )
        self.send(msg)
